//! 邮箱验证码监听：POP3 定时轮询新邮件，提取验证码后走与通知监听相同的
//! 入库 → 广播 → 自动复制管线。
//!
//! 去重策略：SQLite email_seen 表记录已处理邮件的 UIDL；首次成功连接时只把
//! 现存邮件标记为已见（基线），不导入历史邮件——否则启用瞬间可能灌入大量旧码。
//! 更换邮箱账户（host/username）时 update_settings 会清空该表重建基线。
//!
...
fn run(app: &AppHandle, state: &Arc<AppState>) {
    ...
    loop {
        ...
        let cfg = state.settings.read().unwrap().email.clone();

        if !cfg.enabled {
            state.set_email_status(app, "disabled", None);
        ...
        if state.paused.load(Ordering::SeqCst) {
            state.set_email_status(app, "paused", None);
            next_poll = Instant::now();
            continue;
        }
        ...
        match poll_once(app, state, &cfg) {
            Ok(()) => state.set_email_status(app, "running", None),
            Err(e) => {
                eprintln!("邮箱轮询失败: {e}");
                state.set_email_status(app, "error", Some(e));
            }
        }
    }
}
...
fn poll_once(app: &AppHandle, state: &Arc<AppState>, cfg: &EmailSettings) -> Result<(), String> {
...
fn poll_with_client<S: Read + Write>(
    ...
    let seen = state.db.email_seen_set()?;

    match plan_poll(&seen, &uidls) {
        PollPlan::Baseline(marks) => {
            state.db.email_seen_mark(&marks)?;
            eprintln!(
        ...
        PollPlan::Fetch(list) => {
            for (num, uidl) in list {
                ...
                state.db.email_seen_mark(&[uidl])?;
                handle_mail(app, state, &raw);
            }
        }
    }
    Ok(())
...
fn handle_mail(app: &AppHandle, state: &Arc<AppState>, raw: &[u8]) {
    ...
    let username = state.settings.read().unwrap().email.username.clone();
    let source = format!("email:{username}");
