...
SnapCode 是一款 Windows 桌面工具：监听手机通过「手机连接」(Phone Link) 推送到 Windows 的短信 Toast，自动提取其中的验证码，让验证码始终只差一次按键。新验证码到达时立即复制到剪贴板；在任何输入框按下 `Ctrl+Shift+V`，即可把最新一条验证码直接粘贴进去——不用掏手机，不用手打。
...
- **邮箱验证码监听**——支持添加多个邮箱账户，每账户可选 POP3 定时轮询或 IMAP（IDLE 实时推送，服务器不支持 IDLE 时自动降级为轮询），全程 SSL/TLS。从主题与正文中提取验证码，走与短信相同的「入库 → 复制 → 粘贴」管线。邮件按账户在 SQLite 中去重；首次启用只建立基线，不导入历史邮件。
...
可选地，已配置的邮箱账户通过 POP3 或 IMAP 接受监听（rustls TLS + mail-parser 解析 MIME；IMAP 在服务器支持时用 IDLE 实时推送，否则按间隔轮询）：新邮件按账户去重后，邮件主题与正文经过同一个解析器，验证码进入完全相同的管线。
...
| 邮箱验证码 | 多账户管理，POP3 轮询或 IMAP（IDLE 实时推送）识别新邮件验证码（每账户独立 服务器/端口/授权码/间隔/TLS），带测试连接按钮 | 关闭 |
...
SnapCode 完全在本地运行。所有验证码与历史记录仅保存在本机 SQLite 数据库中，不进行任何网络上传，不收集任何数据。随时可通过删除单条记录、清空历史或缩短保留策略来控制数据留存。如配置了邮箱监听，各邮箱账户的授权码同样仅保存在本机 `settings.json`（明文），除对应的邮箱服务器外不会发送到任何地方。
...
- **后端**——Rust：windows-rs（WinRT `UserNotificationListener`、`SendInput`）、rusqlite（内置捆绑 SQLite）、regex、rustls + mail-parser + imap（POP3/IMAP 邮箱监听）
